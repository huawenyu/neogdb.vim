" Whole file borrow from vim-vebugger/autoload/vebugger/util.vim
"
let g:vebugger_breakpoints=[]

"Initialize the default pipe bufferers
function! neobugger#std#setStandardBufferers(debugger)
    for l:pipe in values(a:debugger.pipes)
        let l:pipe.bufferer = function('neobugger#std#readNewLinesFromPipe')
    endfor
endfunction

"Initialize the std part of the debugger's state
function! neobugger#std#setStandardState(debugger)
...
function! neobugger#std#setStandardReadResultTemplate(debugger)
...
function! neobugger#std#setStandardWriteactionsTemplate(debugger)
...
function! neobugger#std#addStandardFunctions(debugger)
...
function! neobugger#std#addStandardThinkHandlers(debugger)
...
function! neobugger#std#addStandardCloseHandlers(debugger)
...
function! neobugger#std#standardInit(debugger)
    call neobugger#std#setStandardBufferers(a:debugger)
    call neobugger#std#setStandardState(a:debugger)
    call neobugger#std#setStandardReadResultTemplate(a:debugger)
    call neobugger#std#setStandardWriteactionsTemplate(a:debugger)
    call neobugger#std#addStandardFunctions(a:debugger)
    call neobugger#std#addStandardThinkHandlers(a:debugger)
    call neobugger#std#addStandardCloseHandlers(a:debugger)
endfunction

"Start a debugger with the std settings
function! neobugger#std#startDebugger(command)
    let l:debugger=neobugger#startDebugger(a:command)

    call neobugger#std#standardInit(l:debugger)

    return l:debugger
endfunction


"Read and return all new lines from a Neobugger pipe object.
function! neobugger#std#readNewLinesFromPipe() dict
...
"process.
function! neobugger#std#openShellBuffer(debugger)
    if has_key(a:debugger,'shellBuffer')
        if -1<bufwinnr(a:debugger.shellBuffer)
            return
        endif
    endif
    let l:oldBuffer=bufnr('Neobugger:Shell')
    if -1<l:oldBuffer
    ...
    setlocal filetype=NeobuggerShell
    setlocal bufhidden=wipe
    let a:debugger.shellBuffer=bufnr('')
    silent file Neobugger:Shell
    wincmd p
endfunction

"Closes the shell buffer
function! neobugger#std#closeShellBuffer(debugger)
    if has_key(a:debugger,'shellBuffer')
        if -1<bufwinnr(a:debugger.shellBuffer)
...
"Set the write actions to add all breakpoints registered in Neobugger
...
function! s:standardThinkHandlers.moveToCurrentLine(readResult,debugger) dict
    if !empty(a:readResult.std.location)
        ...
        if a:debugger.state.std.location!=a:readResult.std.location
            ...
            call neobugger#std#updateMarksForFile(a:debugger.state,a:readResult.std.location.file)
...
function! neobugger#std#updateMarksForFile(state,filename)
...
function! neobugger#std#toggleBreakpoint(file,line)
    let l:debugger=neobugger#getActiveDebugger()
    ...
    for l:i in range(len(g:vebugger_breakpoints))
        ...
        if l:breakpoint.file==a:file && l:breakpoint.line==a:line
            ...
            call neobugger#std#updateMarksForFile(l:debuggerState,a:file)
            return
        endif
    endfor
...
function! neobugger#std#clearBreakpoints()
    let l:debugger=neobugger#getActiveDebugger()
    ...
    for l:file in l:files
        call neobugger#std#updateMarksForFile(l:debuggerState,l:file)
    endfor
endfunction
